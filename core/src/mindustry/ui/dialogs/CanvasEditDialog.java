...
public class CanvasEditDialog extends BaseDialog{
                ...
                public void draw(){
                    ...
                    if(!mobile){
                        ...
                        if(s.x >= 0 && s.y >= 0 && s.x < width && s.y < height){
                            ...
                            Lines.rect(sx + x, sy + y, width / size, height / size, (int)(Lines.getStroke() - 1f));
    ...
    int findClosest(int color){
        ...
        for(int i = 0; i < block.palette.length; i++){
            ...
            float dr = Tmp.c1.r - Tmp.c2.r;
            float dg = Tmp.c1.g - Tmp.c2.g;
            float db = Tmp.c1.b - Tmp.c2.b;
            float dst = dr * dr + dg * dg + db * db;
            if(dst < nearestDst){
                nearest = i;
                nearestDst = dst;
            }
