...
public class ResupplyPoint extends Block{
    ...
    public class ResupplyPointBuild extends Building{
        ...
        public void updateTile(){
            if(timer(timerResupply, resupplyRate / timeScale) && resupply(this, range, ammoAmount, ammoColor)){
                consume();
            }
        }
    }
    ...
    public static boolean resupply(Team team, float x, float y, float range, float ammoAmount, Color ammoColor, Boolf<Unit> valid){
        ...
        Unit unit = Units.closest(team, x, y, range, u -> u.type.ammoType instanceof Item && u.ammo <= u.type.ammoCapacity - ammoAmount && valid.get(u));
        if(unit != null){
